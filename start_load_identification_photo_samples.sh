...
if [ -z $1 ]; then
    ...
    if [ "$#" -ne "6" ]; then
        ...
        JMX_FILE=resources/jmx/identification_many_samples.jmx     # Template jmeter
        ...
        if [ $(find $SAMPLE_DIR -type f -iname "*.octet-stream" | wc -l) -ne $(cat resources/csv_configs/many_biotemplates.csv | wc -l) ]; then
                find $SAMPLE_DIR -type f -iname "*.octet-stream" > resources/csv_configs/many_biotemplates.csv      # Обновить список векторов для методов match и identify
        fi
        
        if [ $(find $SAMPLE_DIR -type f -iname "*.octet-stream" | wc -l) -gt 0 ]; then
            create_metadata                                                                                         # Создать файлы с метаданными для методов add и update
        fi

        if [ $(find $SAMPLE_DIR -type f -iname "*.octet-stream.id" | wc -l) -ne $(cat resources/csv_configs/many_meta.csv | wc -l) ]; then
            find $SAMPLE_DIR -type f -iname "*.octet-stream.id" > resources/csv_configs/many_meta.csv               # Обновить список файлов с метаданными для методов add и update
        fi
