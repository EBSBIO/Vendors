...
f_test_active_liveness_video() {
    ...
    ACTION_TYPE_LIST='(TURN_((RIGHT)|(LEFT)|(UP)|(DOWN)))|(SMILE)|(BLINK)|(DISTORTION)|(RAISE_EYEBROW)|(numbers-digits)'
    DETECT_REGEX_START_PART='\{\s*"passed":\s?(false|true),\s*"score":\s?((0\.0)|(0\.[0-9]*[1-9]+)|(1\.0))\s*(,\s*"results":\s?\[\s*\{\s*"type":\s?"'
    DETECT_REGEX_END_PART='",\s*"passed":\s?(false|true)\s*\}\s*\]\s*)?\}'

    f_regex() {
        LOCAL_META=$1
        ACTION_TYPE=$(echo $LOCAL_META | grep -E "${ACTION_TYPE_LIST}" -o)

        echo ${DETECT_REGEX_START_PART}${ACTION_TYPE}${DETECT_REGEX_END_PART}
    }
    ...
    if [ "$MNEMONIC" == "move" ]; then
        if [[ "$ACTIONS" =~ 1  || "$ACTIONS" == "all" ]]; then
            ...
            f_check -r 200 -m $(f_regex ${META_TURN_RIGHT}) -f "- Score format double is expected"
        ...
        if [[ "$ACTIONS" =~ 2 || "$ACTIONS" == "all" ]]; then
            ...
            f_check -r 200 -m $(f_regex ${META_TURN_LEFT}) -f "- Score format double is expected"
        ...
        if [[ "$ACTIONS" =~ 3 || "$ACTIONS" == "all" ]]; then           
            ...
            f_check -r 200 -m $(f_regex ${META_TURN_UP}) -f "- Score format double is expected"
        ...
        if [[ "$ACTIONS" =~ 4 || "$ACTIONS" == "all" ]]; then
            ...
            f_check -r 200 -m $(f_regex ${META_TURN_DOWN}) -f "- Score format double is expected"
        ...
        if [[ "$ACTIONS" =~ 5 || "$ACTIONS" == "all" ]]; then
            ...
            f_check -r 200 -m $(f_regex ${META_SMILE}) -f "- Score format double is expected"
        ...
        if [[ "$ACTIONS" =~ 6 || "$ACTIONS" == "all" ]]; then
            ...
            f_check -r 200 -m $(f_regex ${META_BLINK}) -f "- Score format double is expected"
        ...
        if [[ "$ACTIONS" =~ 7 || "$ACTIONS" == "all" ]]; then
            ...
            f_check -r 200 -m $(f_regex ${META_DISTORTION}) -f "- Score format double is expected"
        ...
        if [[ "$ACTIONS" =~ 8 || "$ACTIONS" == "all" ]]; then
            ...
            f_check -r 200 -m $(f_regex ${META_RAISE_EYEBROW}) -f "- Score format double is expected"
        ...
        f_check -r 200 -m $(f_regex ${TEXT_META_1}) -f "- Score format double is expected"
        ...
        f_check -r 200 -m $(f_regex ${TEXT_META_2}) -f "- Score format double is expected"
    fi
    ...
    # f_check -r 200 -m $(f_regex ${META}) -f "- Score format double is expected"
    ...
    f_check -r 200 -m $(f_regex ${META_CHARSET_1}) -f "- Score format double is expected"
    ...
    f_check -r 200 -m $(f_regex ${META_CHARSET_2}) -f "- Score format double is expected"
    ...
    f_check -r 200 -m $(f_regex ${META_WF}) -f "- Score format double is expected"
