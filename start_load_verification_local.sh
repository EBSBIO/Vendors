...
if [ -z $1 ]; then
    ...
    if [ "$#" -ne "5" ]; then
        ...
        [ -z $VERSION ] && VERSION="v1"              # Версия API

        REPORT=reports/${1}/${METHOD}_${THREADS}thr_${RAMP}r_$(date "+%Y-%m-%d-%Hh%Mm%Ss")_report.csv  # Отчет по запросам
        PERFLOG=reports/${1}/${METHOD}_${THREADS}thr_${RAMP}r_$(date "+%Y-%m-%d-%Hh%Mm%Ss")_perflog.csv  # Отчет PerfMon
